...
from ..storage.base import StorageInterface, SyncStorageInterface
from ..storage.memory import MemoryStorage, SyncMemoryStorage
...
class Tracker:
    ...
    def _create_storage(self) -> Union[StorageInterface, SyncStorageInterface]:
        ...
        elif storage_type.startswith("postgresql://"):
    ...
    def initialize(self):
        ...
        if not self._initialized:
            if isinstance(self._storage, StorageInterface):
    ...
    def _process_batch(self, items: List[Dict[str, Any]]):
        ...
        if isinstance(self._storage, StorageInterface):
            loop = asyncio.new_event_loop()
            if messages:
                loop.run_until_complete(self._storage.batch_store_messages(messages))
            if operations:
                loop.run_until_complete(self._storage.batch_store_operations(operations))
            loop.close()
        else:
            for msg in messages:
    ...
    def update_operation(self,
        ...
        """Update an existing operation (mainly for completion)."""
        # For now, we create a new record with updated info
        # In a real implementation, we'd update the existing record
    ...
    def extract_and_track(self, *args, **kwargs) -> Dict[str, Any]:
        ...
        for field_path in self.config.track_custom_fields:
            ...
            if not extractor:
                from ..extractors.nested_extractor import PathExtractor
    ...
    def detect_role(self, value: Any, context: Dict[str, Any]) -> str:
        ...
        if isinstance(value, dict):
            from ..extractors.nested_extractor import NestedExtractor
