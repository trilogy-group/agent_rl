...
'''
    ...
    def _extract_dependencies(self, file_path: str, function_name: str) -> str:
        ...
        print(f"DEBUG: Function {function_name} references: {collector.referenced_names}")
        print(f"DEBUG: Function {function_name} calls: {collector.called_functions}")
        ...
        def extract_dependency(name, depth=0):
            ...
            if name in module_definitions and name not in ['logger', 'model', 'tavily']:
                ...
                try:
                    ...
                    print(f"DEBUG: ast.get_source_segment failed for {name}: {e}")
                    ...
                    try:
                        ...
                        if dep_code and dep_code.strip():
                            ...
                            print(f"DEBUG: Could not extract code for {name}")
                    except Exception as e2:
                        print(f"DEBUG: Manual extraction also failed for {name}: {e2}")
                ...
                if isinstance(node, ast.FunctionDef):
                    try:
                        ...
                        print(f"DEBUG: Error analyzing function dependencies for {name}: {e}")
