...
#Analytics #AI #ProductLaunch #DataScience #TechInnovation"""},
    ...
    def _create_evaluator_with_llm(self, tool_name: str, tool_source: str, metadata: Dict, training_data: List[Dict] = None, tool_category: str = "utility") -> str:
        """Use LLM to generate a comprehensive evaluator"""
        
        # Customize system prompt based on tool category
        if tool_category == "natural_language_generation":
            system_prompt = """You are an expert Python developer creating OpenEvolve evaluators for NATURAL LANGUAGE GENERATION tools.
...
Based on your analysis, select 5 specific quality metrics that are most relevant for evaluating this tool's output.
Examples of possible metrics (choose the most relevant ones):
- accuracy, specificity, constructiveness, comprehensiveness, actionability
- clarity, brand_alignment, usability, consistency, depth
- creativity, engagement_potential, relevance, authenticity, impact
- completeness, timeliness, source_quality, coherence, practicality

STEP 3: IMPLEMENT EVALUATION
For NLG tools, you CANNOT use simple string matching. Instead:
1. Use an LLM (like GPT-4o) to evaluate output QUALITY using BLIND assessment
2. DO NOT show the LLM the expected output or characteristics during evaluation
3. Score outputs on your chosen 5 metrics using 0.0-1.0 scale in a SINGLE REQUEST
4. CRITICAL: Use one LLM call per test case to score ALL metrics simultaneously for efficiency
5. Request JSON format responses with STRICT EVALUATION: "Rate each metric 0.0-1.0 where 0.0=poor, 1.0=excellent. Be STRICT and UNFORGIVING - only truly high-quality outputs deserve scores above 0.7. Return format: {"metric1": 0.8, "metric2": 0.7, ...}"
6. Parse the JSON response safely with error handling for malformed JSON
7. Compare scores against minimum thresholds from quality criteria
...
3. Should measure 5 TOOL-SPECIFIC metrics that evaluate THE TOOL'S OUTPUT QUALITY
4. Focus on evaluating the ACTUAL OUTPUT of the tool (e.g., for reflect_on_draft: critique quality, actionability, specificity)
5. Metric keys must be meaningful and specific to what the tool produces
6. Must use LLM-based qualitative assessment for NLG tools, NOT string matching
7. Must handle exceptions gracefully and return all metrics as 0.0 on failure
8. Example for reflect_on_draft: {"critique_quality": 0.8, "actionability": 0.7, "specificity": 0.9, "engagement_assessment": 0.6, "improvement_suggestions": 0.8}

Your evaluator must follow this EXACT structure:
