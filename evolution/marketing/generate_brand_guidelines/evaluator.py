...
EVALUATION_METRICS = ['completeness', 'relevance', 'authenticity', 'actionability']
...
def parse_json_response(response_content: str) -> dict:
    try:
        ...
        if isinstance(parsed, dict):
            ...
            return {k: max(0.0, min(1.0, float(v))) for k, v in parsed.items() if k in EVALUATION_METRICS}
...
def evaluate(program) -> dict:
    ...
    num_cases = len(training_data["brand_research"])

    for input_data in training_data["brand_research"]:
        # Call the tool function
        result = tool_module.generate_brand_guidelines(brand_research=input_data)
